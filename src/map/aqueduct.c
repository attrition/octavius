 ...
 */
static grid_u16 aqueduct;
static grid_u16 aqueduct_backup;
...
{
    map_grid_clear_u16(aqueduct.items);
}

void map_aqueduct_backup(void)
{
    map_grid_copy_u16(aqueduct.items, aqueduct_backup.items);
}

void map_aqueduct_restore(void)
{
    map_grid_copy_u16(aqueduct_backup.items, aqueduct.items);
}

void map_aqueduct_save_state(buffer *buf, buffer *backup)
{
    map_grid_save_state_u16(aqueduct.items, buf);
    map_grid_save_state_u16(aqueduct_backup.items, backup);
}

void map_aqueduct_load_state(buffer *buf, buffer *backup)
{
    map_grid_load_state_u16(aqueduct.items, buf);
    map_grid_load_state_u16(aqueduct_backup.items, backup);
}
