...
void map_grid_init_i16(int16_t *grid, int16_t value)
{
    memset(grid, value, GRID_SIZE * GRID_SIZE * sizeof(int16_t));
}
...
void map_grid_save_state_i16(const int16_t *grid, buffer *buf)
{
    for (int i = 0; i < GRID_SIZE * GRID_SIZE; i++) {
        buffer_write_i16(buf, grid[i]);
    }
}

void map_grid_load_state_i16(int16_t *grid, buffer *buf)
{
    for (int i = 0; i < GRID_SIZE * GRID_SIZE; i++) {
        grid[i] = buffer_read_i16(buf);
    }
}
