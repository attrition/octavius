...
static grid_u16 sprite;
static grid_u16 sprite_backup;
...
{
    map_grid_clear_u16(sprite.items);
}

void map_sprite_backup(void)
{
    map_grid_copy_u16(sprite.items, sprite_backup.items);
}

void map_sprite_restore(void)
{
    map_grid_copy_u16(sprite_backup.items, sprite.items);
}

void map_sprite_save_state(buffer *buf, buffer *backup)
{
    map_grid_save_state_u16(sprite.items, buf);
    map_grid_save_state_u16(sprite_backup.items, backup);
}

void map_sprite_load_state(buffer *buf, buffer *backup)
{
    map_grid_load_state_u16(sprite.items, buf);
    map_grid_load_state_u16(sprite_backup.items, backup);
}
